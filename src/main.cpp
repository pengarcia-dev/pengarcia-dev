...
void loop(void)
{
  while (Serial.available())
  {
    ...
    if (mensaje == 'a')
    {
      ...
      dac_target_mV = round(average_ref - 40);            // Subtract offset if needed
      ...
      actual_dac_voltage = (valorDAC / 255.0) * 3.3 * 1000; // Convert back to mV
    ...
    if (mensaje == 'r')
    {
      ...
      const int N = 200; // Número de muestras
      float voltajes[N]; // Array para almacenar los valores de voltaje

      for (int i = 0; i < N; i++)
      {
        uint64_t sum_voltage = 0;

        // Promedio de varias lecturas para estabilizar la muestra
        for (int j = 0; j < 10; j++)
        {
          raw = adc1_get_raw(ADC1_CHANNEL_6);
          voltage = esp_adc_cal_raw_to_voltage(raw, &adc_chars);
          sum_voltage += voltage;
          delay(1); // Tiempo entre lecturas individuales
        }

        float average = sum_voltage / 10.0 * 986.4 / 1011.11 * 988.4 / 992.05;
        voltajes[i] = average;

        delay(3); // Control del tiempo total entre muestras (~7ms)
      }

      // Mostrar resultados almacenados
      Serial.println("Voltajes medidos:");
      for (int i = 0; i < N; i++)
      {
